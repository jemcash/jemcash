...
*Setup instructions for a Gitian build of Jemcash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Jemcash
...
- [Building Jemcash Core](#building-jemcash-core)
...
Clone the git repositories for Jemcash Core and Gitian.
...
in the Jemcash Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Jemcash Core
----------------

To build Jemcash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Jemcash Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/jemcash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/jemcash/jemcash
...
For example:
```bash
URL=https://github.com/crowning-/jemcash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit jemcash=${COMMIT} --url jemcash=${URL} ../jemcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit jemcash=${COMMIT} --url jemcash=${URL} ../jemcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit jemcash=${COMMIT} --url jemcash=${URL} ../jemcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the jemcash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../jemcash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/jemcash
SIGPATH=/some/root/path/jemcash-detached-sigs

./bin/gbuild --url jemcash=${BTCPATH},signature=${SIGPATH} ../jemcash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/jemcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/jemcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/jemcash-osx-build.assert
```
...
[jemcash/gitian.sigs](https://github.com/jempay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
