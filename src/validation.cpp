...
#if defined(NDEBUG)
# error "Jemcash Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Jemcash Signed Message:\n";
                         ...
                         CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    /// JEMCASH: Check superblock start
    ...
    // JEMCASH : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
            mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
            return state.DoS(10, error("ConnectBlock(JEMCASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                             REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, expectedReward, pindex->nMint, strError)) {
        return state.DoS(0, error("ConnectBlock(JEMCASH): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }}
    ...
    if (!IsBlockPayeeValid(*coinbaseTransaction, pindex->nHeight, expectedReward, pindex->nMint)) {
        return state.DoS(0, error("ConnectBlock(JEMCASH): couldn't find masternode or superblock payments"),
                         REJECT_INVALID, "bad-cb-payee");
    }}

    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(JEMCASH): ProcessSpecialTxsInBlock for block failed with %s", FormatStateMessage(state));
    }
    // END JEMCASH
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 68589){
        ...
        unsigned int nBitsNext = GetNextWorkRequired(pindexPrev, &block, params.GetConsensus());
