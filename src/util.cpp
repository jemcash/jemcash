...
const char * const BITCOIN_CONF_FILENAME = "jemcash.conf";
const char * const BITCOIN_PID_FILENAME = "jemcashd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "jemcash" is a composite category enabling all Jemcash-related debug output
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JemcashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JemcashCore
    // Mac: ~/Library/Application Support/JemcashCore
    // Unix: ~/.jemcashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JemcashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JemcashCore";
#else
    // Unix
    return pathRet / ".jemcashcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty jemcash.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override jemcash.conf
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2017, 2020) + "The Jemcash Core developers \n";
    }
